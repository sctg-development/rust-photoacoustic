 ...
 * ```
 */
function extractBase64Field(configFile: string, fieldPath: string, fieldName: string): string {
    ...
    try {
        ...
        const config: VisualizationConfig = yaml.load(configContent) as VisualizationConfig;
        ...
        for (const part of pathParts) {
            if (!currentObj || typeof currentObj !== 'object' || !(part in currentObj)) {
                throw new Error(`${fieldName} not found in configuration file at path: ${fieldPath}`);
            }
        ...
        if (error instanceof Error) {
            console.error(`Error extracting ${fieldName.toLowerCase()}:`, error.message);
        } else {
 ...
 * ```
 */
export function extractCertAndKey(configFile: string): { cert: string; key: string } {
    return {
