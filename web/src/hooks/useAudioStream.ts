...
export const useAudioStream = (baseUrl?: string): UseAudioStreamReturn => {
    ...
    const updateFps = useCallback(() => {
        ...
        fpsCalculationRef.current = fpsCalculationRef.current.filter(time => time > tenSecondsAgo);
    ...
    const connect = useCallback(async () => {
        ...
        try {
            ...
            console.log(`Connecting to audio stream at ${streamUrl} base url is ${baseUrl}`);
    ...
    }, [isAuthenticated, baseUrl, getAccessToken, isConnecting, isConnected, updateFps]);    // Function to process Server-Sent Events
