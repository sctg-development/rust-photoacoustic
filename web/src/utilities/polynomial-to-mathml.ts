 ...
 *
 * Polynomial to MathML Utility
 *
 * This utility provides functions to convert polynomial coefficients into
 * properly formatted MathML for display in web browsers. It handles
 ...
 * - Converts LaTeX to MathML using Temml library
 *
 ...
 * @returns MathML string representation of the polynomial, or null if invalid input
 *
 * @example
 * // For coefficients [1.5e-3, 0, -2.1, 0, 4.7e-6]
 * // Returns MathML for: 1.5×10⁻³ - 2.1x² + 4.7×10⁻⁶x⁴
 * const mathml = getMathMLFromPolynomialCoefficients([1.5e-3, 0, -2.1, 0, 4.7e-6]);
 *
 * @example
 * // For all zero coefficients [0, 0, 0, 0, 0]
 * // Returns MathML for: 0
 * const mathml = getMathMLFromPolynomialCoefficients([0, 0, 0, 0, 0]);
 */
export const getMathMLFromPolynomialCoefficients = (coefficients: number[]) => {
  try {
    ...
    // Step 3: Convert LaTeX string to MathML using Temml library
    // Temml.renderToString() converts LaTeX math expressions to MathML
    return Temml.renderToString(`${polynomial}`);
  } catch (error) {
    // Handle any errors during LaTeX parsing or MathML generation
    console.error("Error rendering polynomial:", error);

    // Fallback: return a simple bracketed array representation
    // This ensures the UI doesn't break if MathML generation fails
    return `[${coefficients.join(", ")}]`;
  }
};

/**
 * Converts an array of polynomial coefficients to MathML format in classical mathematical order
 *
 ...
 *                      where index 0 is the constant term and index 4 is the highest degree term
 * @returns MathML string representation of the polynomial in classical order, or null if invalid input
 *
 * @example
 * // For coefficients [1.5e-3, 0, -2.1, 0, 4.7e-6]
 * // Returns MathML for: 4.7×10⁻⁶x⁴ - 2.1x² + 1.5×10⁻³
 * const mathml = getMathMLFromPolynomialCoefficientsClassicOrder([1.5e-3, 0, -2.1, 0, 4.7e-6]);
 *
 * @example
 * // For coefficients [5, -3, 0, 2, -1]
 * // Returns MathML for: -x⁴ + 2x³ - 3x + 5
 * const mathml = getMathMLFromPolynomialCoefficientsClassicOrder([5, -3, 0, 2, -1]);
 */
export const getMathMLFromPolynomialCoefficientsClassicOrder = (
  ...
  try {
    ...
    // Handle any errors during LaTeX parsing or MathML generation
    console.error("Error rendering polynomial in classic order:", error);
