...
impl FFTAnalyzer {
    ...
    fn apply_window(&self, signal: &[f32]) -> Vec<f32> {
        ...
        for (i, &sample) in signal.iter().enumerate() {
            let window_factor = match self.window_function {
                ...
                WindowFunction::Hann => 0.5 * (1.0 - (2.0 * std::f32::consts::PI * i as f32 / (signal.len() - 1) as f32).cos()),
                WindowFunction::Blackman => {
                    let a0 = 0.42;
                    let a1 = 0.5;
                    let a2 = 0.08;
                    let x = i as f32 / (signal.len() - 1) as f32;
                    a0 - a1 * (2.0 * std::f32::consts::PI * x).cos() + a2 * (4.0 * std::f32::consts::PI * x).cos()
                },
            };
    ...
    fn compute_fft(&mut self, signal: &[f32]) -> Vec<Complex32> {
        ...
        let mut complex_input: Vec<Complex32> = signal.iter().map(|&x| Complex32::new(x, 0.0)).collect();
...
impl SpectralAnalyzer for FFTAnalyzer {
    ...
    fn get_amplitude_at(&self, frequency: f32) -> Result<f32> {
        let spectrum = self.spectrum_data.as_ref().ok_or_else(|| {
            anyhow::anyhow!("No spectrum data available. Call analyze() first.")
        })?;
