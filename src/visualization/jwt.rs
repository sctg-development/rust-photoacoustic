...
use oxide_auth::primitives::grant::{Grant, Extensions, Value};
use oxide_auth::primitives::issuer::{Issuer, IssuedToken, RefreshedToken, TokenType};
use oxide_auth::primitives::generator::{TagGrant, RandomGenerator};
use url::Url;
...
impl JwtTokenMap {
    ...
    pub fn with_keys(algorithm: Algorithm, encoding_key: EncodingKey, decoding_key: DecodingKey) -> Self {
        JwtTokenMap {
            access_tokens: HashMap::new(),
            refresh_tokens: HashMap::new(),
...
impl Issuer for JwtTokenMap {
    fn issue(&mut self, mut grant: Grant) -> Result<IssuedToken, ()> {
        ...
        // Add to maps
        self.access_tokens.insert(access_token.clone(), Arc::clone(&token_entry));
        if let Some(ref refresh) = refresh_token {
            self.refresh_tokens.insert(refresh.clone(), Arc::clone(&token_entry));
        }
    ...
    fn refresh(&mut self, refresh: &str, mut grant: Grant) -> Result<RefreshedToken, ()> {
        ...
        let (old_access_token, old_refresh_token) = {
            ...
            (token_entry.access_token.clone(), token_entry.refresh_token.clone())
        };
        ...
        let header = Header::new(self.algorithm);
        let new_access_token = encode(&header, &claims, &self.signing_key)
            .map_err(|_| ())?;
        ...
        // Add to maps
        self.access_tokens.insert(new_access_token.clone(), Arc::clone(&new_token_entry));
        if let Some(ref refresh) = new_refresh_token {
            self.refresh_tokens.insert(refresh.clone(), Arc::clone(&new_token_entry));
        }
    ...
    fn recover_token<'a>(&'a self, token: &'a str) -> Result<Option<Grant>, ()> {
        ...
        let exp_time = Utc.timestamp_opt(token_data.claims.exp, 0).single()
            .ok_or(())?;
...
impl JwtIssuer {
    ...
    pub fn debug_token(&self, token: &str) -> Result<JwtClaims, String> {
        ...
        match decode::<JwtClaims>(token, &map.verification_key, &validation) {
            ...
            Err(e) => {
                Err(format!("Failed to decode JWT: {}", e))
            }
        }
    }
