...
mod tests {
    ...
    fn read_stereo_wav_file(path: &str) -> Result<(Vec<f32>, Vec<f32>, u32)> {
        ...
        if num_channels != 2 {
            return Err(anyhow::anyhow!("Expected stereo file (2 channels), found {} channels", num_channels));
        }
    ...
    fn test_differential_with_wav_file() -> Result<()> {
        ...
        assert!(!right_channel.is_empty(), "Right channel should not be empty");
        assert_eq!(left_channel.len(), right_channel.len(), "Channel lengths should be equal");
        
        println!("WAV file loaded: {} samples per channel at {} Hz", left_channel.len(), sample_rate);
        ...
        // Basic validation
        assert_eq!(differential_signal.len(), left_channel.len(), 
        ...
        for i in 0..sample_count {
            ...
            assert_eq!(differential_signal[i], expected, 
                "Differential calculation mismatch at sample {}", i);
        }
        ...
        let diff_mean: f32 = differential_signal.iter().sum::<f32>() / differential_signal.len() as f32;
        ...
        println!("Maximum difference found at sample {}: {}", max_diff_index, max_diff_value);
        println!("Left sample at that point: {}", left_channel[max_diff_index]);
        println!("Right sample at that point: {}", right_channel[max_diff_index]);
        ...
        assert!((actual_diff - max_diff_value).abs() < 1e-6, 
        ...
        save_wav_file(&left_channel, sample_rate, 
            out_dir.join("left_channel.wav").to_str().unwrap())?;
            
        save_wav_file(&right_channel, sample_rate, 
            out_dir.join("right_channel.wav").to_str().unwrap())?;
            
        save_wav_file(&differential_signal, sample_rate, 
