...
mod tests {
    ...
    fn test_bandpass_filter_with_wav_file() {
        ...
        let filter = BandpassFilter::new(2000.0, 100.0)
        ...
        save_wav_file(&samples, sample_rate, 
            out_dir.join("original_signal.wav").to_str().unwrap()).expect("Failed to save original signal");
            
        save_wav_file(&filtered_samples, sample_rate, 
            out_dir.join("filtered_signal_2kHz_100Hz.wav").to_str().unwrap()).expect("Failed to save filtered signal");
    }
    ...
    fn test_bandpass_filter_frequency_response() {
        ...
        for &freq in &test_frequencies {
            ...
            let squared_sum: f32 = filtered_signal.iter()
                .map(|&x| x * x)
                .sum();
        ...
        assert!(amplitudes[2] > amplitudes[0], "Center frequency should be higher than below passband");
        assert!(amplitudes[2] > amplitudes[4], "Center frequency should be higher than above passband");
        ...
        assert!(amplitudes[1] >= amplitudes[2] * 0.5, "Lower edge should have at least 50% amplitude of center");
        assert!(amplitudes[3] >= amplitudes[2] * 0.5, "Upper edge should have at least 50% amplitude of center");
        
        // Check that frequencies outside the passband are significantly attenuated
        assert!(amplitudes[0] < amplitudes[2] * 0.5, "Below passband should be attenuated");
        assert!(amplitudes[4] < amplitudes[2] * 0.5, "Above passband should be attenuated");
        ...
        for (i, &freq) in test_frequencies.iter().enumerate() {
            ...
            save_wav_file(&test_signal, sample_rate, 
                out_dir.join(format!("original_{}_Hz.wav", freq)).to_str().unwrap())
                .expect(&format!("Failed to save original signal at {} Hz", freq));
                
            save_wav_file(&filtered_signal, sample_rate, 
                out_dir.join(format!("filtered_{}_Hz.wav", freq)).to_str().unwrap())
