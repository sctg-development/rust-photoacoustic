...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "Connecting to photoacoustic Modbus server at {}",
        socket_addr
    );
    ...
    println!(
        "Input register 0 (frequency): {:.1} Hz",
        input_regs[0] as f32 / 10.0
    );
    println!(
        "Input register 1 (amplitude): {:.3}",
        input_regs[1] as f32 / 1000.0
    );
    println!(
        "Input register 2 (concentration): {:.1} ppm",
        input_regs[2] as f32 / 10.0
    );
