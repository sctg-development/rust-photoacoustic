...
- **Audit du code** : Pour chaque service, tracer la façon dont la config est lue (hot-reload ou non).
- **Tests** : Ajouter des tests unitaires/fonctionnels pour valider le comportement dynamique de la config.
- **API RESTful** : Ajouter un endpoint GET `/api/config` pour introspection.
- **Sécurité** : Authentifier l’accès à `/api/config` (token, role admin…).

---

## 8. Liens utiles

- [Gestion d’état et validation dans le projet](https://github.com/sctg-development/rust-photoacoustic/blob/main/rust/docs_conception_fr/rust_patterns_guide.md)
- [Rocket Managed State](https://rocket.rs/v0.5-rc/guide/state/)
- [Code source Config](https://github.com/sctg-development/rust-photoacoustic/blob/main/rust/src/config/mod.rs)

---

_**Ce rapport est basé sur un audit partiel du code (limite de 10 fichiers par recherche). Pour une couverture complète, voir la recherche [GitHub Code Search](https://github.com/sctg-development/rust-photoacoustic/search?q=config)._ 