...
// Get the correct path to fixtures directory relative to the test file location
const fixturesDir = path.join(__dirname, '..', 'fixtures', 'requests');

const inputFileNames = readdirSync(fixturesDir, 'utf-8');

const fixtures: [string, Request][] = inputFileNames.map(inputFileName => [
  inputFileName.replace('.json', ''),
  JSON.parse(readFileSync(path.join(fixturesDir, inputFileName)).toString()),
]);
...
// TODO: Tests for request validation have been disabled - they require proper fixture files
// availableTargets()
...
describe('addTargetClient', () => {
  it('should add a new custom target', () => {
    ...
    const snippet = new HTTPSnippet(short as any);
