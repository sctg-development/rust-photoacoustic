...
async fn test_microphone_chunk_timing() -> Result<()> {
    ...
    config.frame_size = 2048; // Smaller window for faster response
    ...
    println!(
        "Target chunk size: approximately 20ms = {} samples",
        (sample_rate as f32 * 0.02) as usize
    );
    ...
    for i in 0..20 {
        ...
        match mic_source.read_frame() {
            Ok((channel_a, channel_b)) => {
                ...
                println!(
                    "Frame {}: {} samples/channel, took {:.1}ms",
                    i + 1,
                    channel_a.len(),
                    frame_duration.as_micros() as f64 / 1000.0
                );
    ...
    if !frame_times.is_empty() {
        ...
        println!(
            "  Total test duration: {:.1}ms",
            total_duration.as_micros() as f64 / 1000.0
        );
        println!("  Frames processed: {}", frame_times.len());
        println!(
            "  Average frame time: {:.1}ms",
            avg_frame_time.as_micros() as f64 / 1000.0
        );
        println!(
            "  Min frame time: {:.1}ms",
            min_frame_time.as_micros() as f64 / 1000.0
        );
        println!(
            "  Max frame time: {:.1}ms",
            max_frame_time.as_micros() as f64 / 1000.0
        );
