...
async fn test_protected_api_with_jwt() {
    ...
    let token_data: Value = serde_json::from_str(
        &oauth_response.into_string().await.expect("OAuth2 token response")
    ...
    let profile_data: Value = serde_json::from_str(
        &profile_response.into_string().await.expect("Profile response")
    ).expect("Valid JSON response");
    
    assert!(profile_data.get("user_id").is_some(), "Profile should contain user_id");
